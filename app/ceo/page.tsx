...
import {
  ...
  AlertCircle, TrendingUp, RefreshCcw, Lock, Unlock, Percent, Banknote, Map, Package, LogOut
} from "lucide-react";
...
const INITIAL_EMPLOYEES = [
  {
    id: "EMP-100", name: "محمد العماري", phone: "0921234567", dob: "1994-05-12", avatar: "", documentId: "",
    branch: "فرع الحدائق", role: "كاشير توصيل", accessLevel: "كاشير", pin: "1234", baseSalary: 1200,
    deductHalfDay: 20, deductFullDay: 40, mealsEnabled: true, mealsLimit: 150,
    cashWithdrawEnabled: true, cashWithdrawLimit: 200, totalAdvance: 1000, monthlyAdvanceCut: 150,
    createdAt: new Date().toISOString()
  },
  {
    id: "EMP-101", name: "أحمد الفيتوري", phone: "0917654321", dob: "1990-11-20", avatar: "", documentId: "",
    branch: "فرع وسط البلاد", role: "مدير مناوب", accessLevel: "مدير فرع", pin: "5555", baseSalary: 2500,
    deductHalfDay: 40, deductFullDay: 80, mealsEnabled: true, mealsLimit: 300,
    cashWithdrawEnabled: false, cashWithdrawLimit: 0, totalAdvance: 0, monthlyAdvanceCut: 0,
    createdAt: new Date().toISOString()
  }
];
...
export default function CEODashboard() {
  ...
  const [branchesControl, setBranchesControl] = useState([
    { id: "B1", name: "فرع وسط البلاد", active: true },
    { id: "B2", name: "فرع الحدائق", active: true },
    { id: "B3", name: "فرع فينيسيا", active: false },
  ]);

  const [systemSettings, setSystemSettings] = useState({ deliveryFee: 10, taxRate: 0, hospitalityLimit: 150 });
  const [successMsg, setSuccessMsg] = useState("");

  // تأسيس بيانات الموظفين إذا كانت فارغة
  useEffect(() => {
    db.employees.count().then(count => {
      if (count === 0) db.employees.bulkAdd(INITIAL_EMPLOYEES);
    });
  }, []);
  ...
  return (
    <div className="flex h-screen bg-[#0A0F1C] text-gray-100 font-sans" dir="rtl">

      <AnimatePresence>
        {successMsg && (
          <motion.div initial={{ y: -50, opacity: 0 }} animate={{ y: 20, opacity: 1 }} exit={{ y: -50, opacity: 0 }}
            className="absolute top-0 left-1/2 -translate-x-1/2 z-50 bg-emerald-500 text-white px-6 py-3 rounded-full font-bold shadow-lg shadow-emerald-500/20 flex items-center gap-2">
      ...
      <aside className="w-72 bg-[#121A2F] border-l border-gray-800 flex flex-col shrink-0 z-20">
        <div className="p-6 border-b border-gray-800 flex items-center gap-4">
          <div className="w-12 h-12 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-2xl flex items-center justify-center shadow-lg shadow-indigo-500/20 relative overflow-hidden">
          ...
          <div>
            <h1 className="text-xl font-extrabold text-white tracking-tight">منصة CEO</h1>
            <p className="text-[10px] text-indigo-400 font-bold uppercase tracking-widest mt-1"><ShieldCheck className="w-3 h-3 inline pb-0.5" /> تحكم إداري كامل</p>
          </div>
        </div>
          ...
          {[
            ...
            return (
              <button key={tab.id} onClick={() => setActiveTab(tab.id as TabType)}
                className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-2xl transition-all font-bold text-sm ${isActive ? "bg-indigo-500/10 text-indigo-400 shadow-sm border border-indigo-500/20" : "text-gray-400 hover:bg-[#1A243D] hover:text-gray-200"
                  }`}>
      ...
      {/* المحتوى */}
      <main className="flex-1 flex flex-col h-full overflow-hidden bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] bg-blend-overlay bg-[#0A0F1C]/95">
        <header className="h-20 border-b border-gray-800 shrink-0 flex items-center px-8 justify-between relative overflow-hidden">
          <h2 className="text-2xl font-extrabold text-white relative z-10">{activeTab}</h2>
          <div className="flex items-center gap-3 relative z-10">
            <div className="text-left bg-gray-900/50 px-4 py-2 rounded-xl border border-gray-800">
              <p className="text-xs font-bold text-gray-400">Chief Executive Officer</p>
              <p className="text-sm font-extrabold text-white">المدير العام</p>
            </div>
            <img src="https://i.pravatar.cc/150?u=a042581f4e29026704d" alt="CEO" className="w-12 h-12 rounded-full border-2 border-indigo-500 shadow-lg shadow-indigo-500/20" />
            <button
              onClick={() => { clearSession(); window.location.href = "/"; }}
              title="تسجيل الخروج"
          ...
          <div className="max-w-7xl mx-auto">
            ...
            {activeTab === "إعدادات الصلاحيات" && (
              <SystemControlSettingsView
                ...
                toggleBranch={(id: string) => setBranchesControl(prev => prev.map(b => b.id === id ? { ...b, active: !b.active } : b))}
...
function SystemControlSettingsView({ settings, setSettings, branchesControl, toggleBranch, showSuccess }: any) {
  const update = (f: string, v: any) => setSettings((p: any) => ({ ...p, [f]: v }));

  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center bg-[#121A2F] p-6 rounded-3xl border border-gray-800 shadow-xl">
        <h3 className="text-xl font-extrabold text-white flex items-center gap-2"><Settings className="w-6 h-6 text-indigo-400" /> الإعدادات المتقدمة وضوابط النظام والتشغيل</h3>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {/* 1. نظام إيقاف وتشغيل الفروع */}
        <div className="bg-[#121A2F] border border-gray-800 rounded-3xl p-6 shadow-2xl relative overflow-hidden">
          <div className="absolute -right-10 top-0 opacity-5"><Building className="w-40 h-40" /></div>
          <h4 className="text-sm font-extrabold text-white mb-6 flex items-center gap-2 border-b border-gray-800 pb-3 relative z-10">إدارة التشغيل المركزية للفروع (الربط المباشر)</h4>

          <div className="space-y-4 relative z-10">
            {branchesControl.map((b: any) => (
              <div key={b.id} className="flex justify-between items-center p-4 rounded-xl border border-gray-800 bg-[#0A0F1C]">
                <div>
                ...
                <button onClick={() => {
                  ...
                  className={`flex items-center gap-2 px-4 py-2 font-bold text-xs rounded-xl transition ${b.active ? 'bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white' : 'bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500 hover:text-white'}`}>
        ...
        <div className="bg-[#121A2F] border border-gray-800 rounded-3xl p-6 shadow-2xl">
          <h4 className="text-sm font-extrabold text-white mb-6 flex items-center gap-2 border-b border-gray-800 pb-3"><Settings className="w-4 h-4 text-indigo-400" /> قواعد التجارة الثابتة (تطبع لكل الفروع)</h4>
          <div className="space-y-6">
            <div>
              ...
              <input type="number" value={settings.deliveryFee} onChange={e => update('deliveryFee', Number(e.target.value))} className="w-full bg-[#0A0F1C] border border-gray-800 rounded-xl px-4 py-3 outline-none font-mono text-gray-200 font-bold" />
            </div>
            <div>
              ...
              <input type="number" value={settings.taxRate} onChange={e => update('taxRate', Number(e.target.value))} className="w-full bg-[#0A0F1C] border border-gray-800 rounded-xl px-4 py-3 outline-none font-mono text-gray-200 font-bold" />
            </div>
            <div>
            ...
            <button onClick={() => showSuccess("تم اعتماد الأسعار الثابتة وأُرسلت لجميع كاشيرية الفروع.")} className="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-3.5 rounded-xl transition flex items-center justify-center gap-2">
      ...
      <div className="bg-red-500/5 border border-red-500/20 rounded-3xl p-6 shadow-2xl">
        ...
        <button onClick={() => { if (confirm("مستحيل الاسترجاع! هل تود تفريغ الخزائن وبدء داتا مالية جديدة؟")) showSuccess("تم تصفير الدفاتر."); }} className="bg-red-600 hover:bg-red-500 text-white font-bold px-6 py-3 rounded-xl transition text-sm flex items-center gap-2 shadow-lg shadow-red-600/20">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-[#121A2F] border border-emerald-500/30 p-6 rounded-3xl shadow-lg relative overflow-hidden group">
          ...
          <p className="text-4xl font-mono font-black text-white">{globalRev.toLocaleString(undefined, { maximumFractionDigits: 0 })} <span className="text-sm text-emerald-500">د.ل</span></p>
        </div>
        <div className="bg-[#121A2F] border border-indigo-500/30 p-6 rounded-3xl shadow-lg relative overflow-hidden group">
          <div className="absolute right-0 top-0 p-4 opacity-5 group-hover:scale-110 transition duration-500"><Wallet className="w-24 h-24 text-indigo-500" /></div>
          <h4 className="text-[11px] font-extrabold tracking-widest uppercase text-indigo-400/80 mb-2">عدد الطلبات الكلي</h4>
          <p className="text-4xl font-mono font-black text-white">{orders.length} <span className="text-sm text-indigo-500">طلب</span></p>
        </div>
        <div className="bg-[#121A2F] border border-orange-500/30 p-6 rounded-3xl shadow-lg relative overflow-hidden group">
          ...
          <p className="text-4xl font-mono font-black text-white">{empCount} <span className="text-sm text-orange-500">موظف</span></p>
        </div>
        <div className="bg-[#121A2F] border-l-4 border-red-500 border-gray-800 p-6 rounded-3xl shadow-lg flex flex-col justify-center">
          ...
          <p className="text-sm font-bold text-gray-300">يوجد <span className="text-red-400 font-extrabold text-xl">{lowStock.length}</span> صنف بحاجة توريد عاجل.</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* رادار أداء الفروع */}
        <div className="lg:col-span-2 bg-[#121A2F] border border-gray-800 rounded-3xl p-6 shadow-2xl">
          <h3 className="text-lg font-extrabold text-white mb-6 flex items-center gap-2"><Building className="w-5 h-5 text-indigo-400" /> رادار أداء ونبض الفروع</h3>
          <div className="space-y-4">
            {branchesControl.map((b: any) => (
              <div key={b.id} className={`p-5 rounded-2xl border flex items-center justify-between transition ${b.active ? 'bg-indigo-500/5 border-indigo-500/20' : 'bg-red-500/5 border-red-500/20 opacity-70'}`}>
                <div className="flex items-center gap-4">
                  <div className={`w-3 h-3 rounded-full ${b.active ? 'bg-emerald-500 shadow-[0_0_10px_#10b981] animate-pulse' : 'bg-red-500'}`}></div>
                  <div>
                    <h4 className="text-base font-extrabold text-white mb-0.5">{b.name}</h4>
                ...
                <div className="flex gap-6 items-center text-left">
                  ...
                  <div>
                    ...
                    <p className="font-mono font-extrabold text-indigo-400">{orders.filter((o: any) => o.branchId === b.id).length}</p>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="bg-[#121A2F] border border-gray-800 rounded-3xl p-6 shadow-2xl flex flex-col">
          <h3 className="text-lg font-extrabold text-white mb-6 flex items-center gap-2"><AlertCircle className="w-5 h-5 text-orange-400" /> إنذار نواقص المستودع (Live)</h3>
                  ...
                  <div key={item.id} className={`${isCritical ? 'bg-red-500/10 border-red-500/20' : 'bg-orange-500/10 border-orange-500/20'} border rounded-xl p-4 flex gap-3 items-start`}>
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-[10px] text-gray-400 leading-relaxed font-medium">
                        المتبقي: <strong className="text-white">{item.qty} {item.unit}</strong> من أصل <strong className="text-gray-300">{item.min}</strong> حد أدنى مطلوب ({pct}%)
                      </p>
          ...
          <div className="mt-4 pt-4 border-t border-gray-800">
            <button className="w-full bg-[#1A243D] hover:bg-[#2A3757] text-white font-bold text-xs py-3 rounded-lg transition">
...
function PurchasesApprovalView({ invoices, inventory, onApprove }: {
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center bg-[#121A2F] p-6 rounded-3xl border border-gray-800 shadow-xl">
        <div>
          <h3 className="text-xl font-extrabold text-white">اعتماد وتأكيد المشتريات والتوريد</h3>
      ...
      <div className="bg-[#121A2F] border border-gray-800 rounded-3xl overflow-hidden shadow-2xl">
        <div className="px-6 py-4 border-b border-gray-800 bg-[#1A243D]">
          <h3 className="text-sm font-extrabold text-white">سجل فواتير التوريد المعلقة للموافقة</h3>
        </div>
        ...
        {invoices.length === 0 ? (
          ...
          <table className="w-full text-right border-collapse">
            <thead className="bg-[#1A243D] border-b border-gray-800 text-gray-400">
              <tr>
            ...
            <tbody className="divide-y divide-gray-800">
              {invoices.map((inv: any) => {
                ...
                return (
                  <tr key={inv.id} className="hover:bg-[#1A243D]/50 transition">
                    <td className="px-6 py-6 text-xs font-mono font-bold text-indigo-400">{inv.id}</td>
                    <td className="px-6 py-6 text-sm font-extrabold text-white">
                      {inv.supplier}
                    ...
                    <td className="px-6 py-6 text-sm font-mono font-bold text-center text-gray-300">
                    ...
                    <td className="px-6 py-6 text-center">
                      <button
                        ...
                        disabled={isLoading}
                        className="bg-emerald-600 hover:bg-emerald-500 disabled:opacity-60 text-white font-bold text-xs px-4 py-2.5 rounded-xl shadow-lg shadow-emerald-600/20 transition flex items-center justify-center gap-1.5 w-full"
                      >
                        {isLoading ? (
...
function EmployeesDirectoryView({ employees, onAddClick, onEmpClick }: any) {
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center bg-[#121A2F] p-6 rounded-3xl border border-gray-800 shadow-xl">
        <div>
          <h3 className="text-xl font-extrabold text-white">الكادر الوظيفي والهيكل</h3>
          <p className="text-sm text-gray-400 font-medium mt-1">إجمالي الموظفين المسجلين حالياً: <strong className="text-indigo-400">{employees.length}</strong> موظف.</p>
        </div>
        <button onClick={onAddClick} className="bg-indigo-600 hover:bg-indigo-500 text-white px-6 py-3.5 rounded-2xl font-bold flex items-center gap-2 transition shadow-lg shadow-indigo-600/20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {employees.map((emp: Employee) => (
          <div key={emp.id} onClick={() => onEmpClick(emp)} className="bg-[#121A2F] border border-gray-800 hover:border-indigo-500/50 rounded-3xl p-6 cursor-pointer transition relative overflow-hidden group hover:shadow-[0_0_30px_rgba(99,102,241,0.1)]">
            ...
            <div className="flex gap-4 items-center">
              <div className="w-16 h-16 bg-gray-800 rounded-2xl shrink-0 flex items-center justify-center overflow-hidden border border-gray-700 relative">
              ...
              <div className="flex-1 w-full truncate">
                <div className="flex justify-between items-start mb-1">
                  <h4 className="font-extrabold text-white text-lg truncate flex-1">{emp.name}</h4>
                  <span className="text-[10px] bg-indigo-500/20 text-indigo-300 px-2 py-0.5 rounded ml-2 whitespace-nowrap">{emp.id}</span>
                </div>
            ...
            <div className="mt-5 pt-5 border-t border-gray-800 flex justify-between items-center text-sm font-bold text-gray-500 group-hover:text-indigo-400 transition">
...
function AddEmployeeForm({ onCancel, onSubmit }: any) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSubmit({
      ...
      avatar: hasAvatar ? "avatar_url.png" : "",
      documentId: hasDoc ? "passport_scan.png" : ""
    });
  };
  ...
  return (
    <div className="bg-[#121A2F] border border-gray-800 rounded-3xl overflow-hidden shadow-2xl relative">
      {/* Header */}
      <div className="bg-[#1A243D] px-8 py-6 border-b border-gray-800 flex justify-between items-center">
        <div>
          <h3 className="text-xl font-extrabold text-white flex items-center gap-2 mb-1"><UserPlus className="w-6 h-6 text-indigo-400" /> استمارة تعيين وتخصيص موظف جديد</h3>
      ...
      <form onSubmit={handleSubmit} className="p-8 space-y-12">
        ...
        <div>
          <h4 className="text-base font-extrabold text-indigo-400 mb-6 flex items-center gap-2 border-b border-gray-800 pb-3">
            <span className="bg-indigo-500/20 w-8 h-8 flex items-center justify-center rounded-lg text-indigo-300">1</span>
            البيانات الشخصية والثبوتية
          </h4>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              <label className="text-xs font-bold text-gray-400 block mb-2">اسم الموظف الرباعي</label>
              <input type="text" value={formData.name} onChange={e => update('name', e.target.value)} required placeholder="إدخال الاسم..." className="w-full bg-[#0A0F1C] border border-gray-700 focus:border-indigo-500 rounded-xl px-4 py-3 outline-none font-bold text-gray-100" />
            </div>
            <div>
              <label className="text-xs font-bold text-gray-400 block mb-2">رقم الهاتف (للتواصل السريع)</label>
              <input type="tel" value={formData.phone} onChange={e => update('phone', e.target.value)} required placeholder="09X XXX XXXX" className="w-full bg-[#0A0F1C] border border-gray-700 focus:border-indigo-500 rounded-xl px-4 py-3 outline-none font-bold text-gray-100" dir="ltr" />
            </div>
            <div>
              <label className="text-xs font-bold text-gray-400 block mb-2">تاريخ الميلاد</label>
              <input type="date" value={formData.dob} onChange={e => update('dob', e.target.value)} required className="w-full bg-[#0A0F1C] border border-gray-700 focus:border-indigo-500 rounded-xl px-4 py-3 outline-none font-bold text-gray-100" />
            </div>
          </div>

          {/* المرفقات - محاكاة واجهة الأزرار */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
            <button type="button" onClick={() => setHasAvatar(!hasAvatar)} className={`w-full py-4 rounded-2xl border-2 border-dashed flex items-center justify-center gap-3 transition font-bold text-sm ${hasAvatar ? 'border-emerald-500 bg-emerald-500/10 text-emerald-400' : 'border-gray-700 bg-gray-800/50 text-gray-400 hover:border-gray-500'}`}>
              <ImageIcon className="w-5 h-5" /> {hasAvatar ? 'الصورة الشخصية تم إرفاقها ✓' : 'إرفاق صورة شخصية للموظف (محاكاة)'}
            </button>
            <button type="button" onClick={() => setHasDoc(!hasDoc)} className={`w-full py-4 rounded-2xl border-2 border-dashed flex items-center justify-center gap-3 transition font-bold text-sm ${hasDoc ? 'border-emerald-500 bg-emerald-500/10 text-emerald-400' : 'border-gray-700 bg-gray-800/50 text-gray-400 hover:border-gray-500'}`}>
              <FileBadge className="w-5 h-5" /> {hasDoc ? 'إثبات الهوية تم إرفاقه (جواز/بطاقة) ✓' : 'تصوير وإرفاق مستند الهوية (محاكاة)'}
            </button>
          </div>
        </div>
        ...
        <div>
          <h4 className="text-base font-extrabold text-indigo-400 mb-6 flex items-center gap-2 border-b border-gray-800 pb-3">
            <span className="bg-indigo-500/20 w-8 h-8 flex items-center justify-center rounded-lg text-indigo-300">2</span>
            الصلاحية وتوجيه الفرع
          </h4>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
            <div>
              <label className="text-xs font-bold text-gray-400 block mb-2">الدور / المسمى الوظيفي</label>
              <input type="text" value={formData.role} onChange={e => update('role', e.target.value)} required placeholder="مثال: عامل نظافة، متلقي طلبات، شيف..." className="w-full bg-[#0A0F1C] border border-gray-700 focus:border-indigo-500 rounded-xl px-4 py-3 outline-none font-bold text-gray-100" />
            </div>
            <div>
              <label className="text-xs font-bold text-gray-400 block mb-2">توجيه إلى فرع</label>
              <select value={formData.branch} onChange={e => update('branch', e.target.value)} className="w-full bg-[#0A0F1C] border border-gray-700 focus:border-indigo-500 rounded-xl px-4 py-3 outline-none font-bold text-gray-100">
                {BRANCHES.map(b => <option key={b} value={b}>{b}</option>)}
              </select>
            </div>
            <div className="bg-[#1A243D] p-3 rounded-xl border border-gray-700 flex flex-col justify-center">
              <label className="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" checked={needsLogin} onChange={e => setNeedsLogin(e.target.checked)} className="w-5 h-5 accent-indigo-500" />
                <span className="text-sm font-bold text-indigo-300">هذه الوظيفة تتطلب تسجيل الدخول للنظام (كاشير، إدارة)</span>
              </label>
            </div>
              ...
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="md:col-span-3 pt-2">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-gray-800 pt-6">
                  <div>
                    ...
                    <select value={formData.accessLevel} onChange={e => update('accessLevel', e.target.value)} required={needsLogin} className="w-full bg-[#0A0F1C] border border-orange-500/50 focus:border-orange-500 rounded-xl px-4 py-3 outline-none font-bold text-white">
                      <option value="">-- اختار نوع لوحة التحكم الخاصة به --</option>
                      <option value="واجهة الكاشير">واجهة الكاشير (POS)</option>
                      <option value="لوحة مدير الفرع">لوحة تحكم مدير الفرع (Dashboard)</option>
                      <option value="المخازن">لوحة أمين المخزن المركزية</option>
                      <option value="الخزنة والمالية">إدارة الخزينة والمحاسبة</option>
                      <option value="الإدارة العليا (CEO)">الإدارة العليا (CEO)</option>
                    </select>
                  </div>
                  <div>
        ...
        <div>
          <h4 className="text-base font-extrabold text-indigo-400 mb-6 flex items-center gap-2 border-b border-gray-800 pb-3">
            <span className="bg-indigo-500/20 w-8 h-8 flex items-center justify-center rounded-lg text-indigo-300">3</span>
            البناء المحاسبي (الرواتب، السقف، الديون)
          </h4>

          <div className="bg-[#0A0F1C] border border-gray-800 rounded-2xl p-6 mb-8">
            <h5 className="text-sm font-bold text-emerald-400 mb-4 pb-2 border-b border-gray-800">قيمة الدخل والحسم الأساسي (تلقائي الحساب كل شهر)</h5>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                <label className="text-xs font-bold text-gray-400 block mb-2">قيمة الراتب الأساسي الإجمالي</label>
                <div className="relative">
                  <input type="number" min="0" value={formData.baseSalary === 0 ? "" : formData.baseSalary} onChange={e => update('baseSalary', Number(e.target.value))} required placeholder="0" className="w-full bg-[#121A2F] border border-gray-700 focus:border-emerald-500 rounded-xl pl-12 pr-4 py-3 outline-none font-mono text-lg text-white" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
            ...
            <div className="bg-[#0A0F1C] border border-gray-800 rounded-2xl p-6">
              <div className="flex justify-between items-center border-b border-gray-800 pb-4 mb-4">
                <h5 className="text-sm font-bold text-white flex items-center gap-2"><Coffee className="w-4 h-4 text-orange-400" /> سحب الوجبات وإطعام الموظف</h5>
              ...
              <div className={formData.mealsEnabled ? 'opacity-100' : 'opacity-30 pointer-events-none'}>
                ...
                <input type="number" value={formData.mealsLimit === 0 ? "" : formData.mealsLimit} onChange={e => update('mealsLimit', Number(e.target.value))} placeholder="قيمة السقف المسموح" className="w-full bg-[#121A2F] border border-gray-700 rounded-xl px-4 py-3 outline-none font-mono text-gray-200" />
            ...
            <div className="bg-[#0A0F1C] border border-gray-800 rounded-2xl p-6">
              <div className="flex justify-between items-center border-b border-gray-800 pb-4 mb-4">
                <h5 className="text-sm font-bold text-white flex items-center gap-2"><Wallet className="w-4 h-4 text-emerald-400" /> صرف سحب نقدي (مصروف جيب)</h5>
              ...
              <div className={formData.cashWithdrawEnabled ? 'opacity-100' : 'opacity-30 pointer-events-none'}>
                ...
                <input type="number" value={formData.cashWithdrawLimit === 0 ? "" : formData.cashWithdrawLimit} onChange={e => update('cashWithdrawLimit', Number(e.target.value))} placeholder="قيمة السقف المستقطع النقدي المسموح" className="w-full bg-[#121A2F] border border-gray-700 rounded-xl px-4 py-3 outline-none font-mono text-gray-200" />
          ...
          <div className="bg-indigo-900/10 border border-indigo-500/20 rounded-2xl p-6">
            <h5 className="text-sm font-bold text-indigo-300 mb-4 pb-2 border-b border-indigo-900/50 flex items-center gap-2"><CreditCard className="w-4 h-4" /> سلفة الموظف (تأسيس دين مؤجل إن وجد)</h5>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="text-[10px] font-bold text-indigo-200 block mb-2">إجمالي قيمة السلفة المطلوبة/الحالية (د.ل)</label>
                <input type="number" value={formData.totalAdvance === 0 ? "" : formData.totalAdvance} onChange={e => update('totalAdvance', Number(e.target.value))} placeholder="قيمة الدين الكامل" className="w-full bg-[#0A0F1C] border border-indigo-500/30 focus:border-indigo-500 rounded-xl px-4 py-3 outline-none font-mono text-indigo-300" />
              </div>
              <div>
                <label className="text-[10px] font-bold text-pink-300 block mb-2">القسط القابل للخصم الشهري تلقائياً (د.ل)</label>
                <input type="number" value={formData.monthlyAdvanceCut === 0 ? "" : formData.monthlyAdvanceCut} onChange={e => update('monthlyAdvanceCut', Number(e.target.value))} placeholder="قيمة الاستقطاع في تسوية رواتب كل شهر" className="w-full bg-[#0A0F1C] border border-pink-500/30 focus:border-pink-500 rounded-xl px-4 py-3 outline-none font-mono text-pink-300" />
              </div>
            </div>
            {formData.totalAdvance > 0 && formData.monthlyAdvanceCut > 0 && (
              <p className="text-[10px] font-bold text-indigo-400 mt-4 bg-indigo-500/10 p-3 rounded-lg border border-indigo-500/20">
        ...
        <div className="pt-8 border-t border-gray-800 flex justify-end gap-4">
          <button type="button" onClick={onCancel} className="px-8 py-4 font-bold text-gray-400 hover:text-white transition">إلغاء التعيين</button>
          <button type="submit" className="bg-indigo-600 hover:bg-indigo-500 text-white px-12 py-4 rounded-xl font-bold flex items-center gap-2 shadow-lg shadow-indigo-600/30 transition">
...
function EmployeeReceiptDetails({ employee, onClose, onDelete }: any) {
  ...
  return (
    <div className="bg-[#121A2F] border border-gray-800 rounded-3xl overflow-hidden shadow-2xl relative max-w-4xl mx-auto">
      <div className="bg-[#1A243D] px-8 py-6 border-b border-gray-800 flex justify-between items-center">
        <h3 className="text-xl font-extrabold text-white flex items-center gap-2">
          <FileBadge className="w-6 h-6 text-indigo-400" /> الإيصال الوظيفي وكشف الراتب التفصيلي
        </h3>
      ...
      <div className="p-8">
        <div className="flex justify-between items-start mb-8 pb-8 border-b border-gray-800 border-dashed">
          <div className="flex gap-5">
            <div className="w-20 h-20 bg-[#0A0F1C] border-2 border-indigo-500 rounded-2xl flex items-center justify-center">
              <Users className="w-10 h-10 text-indigo-500" />
            </div>
            <div>
              <h2 className="text-2xl font-extrabold text-white mb-1">{employee.name}</h2>
              <p className="text-sm font-bold text-indigo-400">{employee.role} — {employee.branch}</p>
          ...
          <div className="text-center bg-[#0A0F1C] p-4 rounded-2xl border border-gray-800">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          ...
          <div>
            <h4 className="text-sm font-extrabold text-gray-300 mb-6 uppercase tracking-widest bg-gray-800/30 px-3 py-1.5 inline-block rounded">معادلة المعاش</h4>
            <div className="space-y-4 text-sm font-bold">
              ...
              <div className="flex justify-between items-center text-orange-400 border-t border-gray-800/50 pt-4 mt-4">
          ...
          <div>
            <h4 className="text-sm font-extrabold text-gray-300 mb-6 uppercase tracking-widest bg-gray-800/30 px-3 py-1.5 inline-block rounded">موقف السقوف التشغيلية والاستهلاك</h4>
            ...
            <div className="space-y-6">
              ...
              <div>
                <div className="flex justify-between items-end mb-2">
                  ...
                  <p className="text-[10px] font-mono font-bold text-indigo-400">استهلك {testUsedMeals} / {employee.mealsLimit} د.ل</p>
                </div>
                <div className={`h-2 bg-[#0A0F1C] rounded-full overflow-hidden ${!employee.mealsEnabled && 'opacity-30'}`}>
              ...
              <div>
                <div className="flex justify-between items-end mb-2">
                  ...
                  <p className="text-[10px] font-mono font-bold text-indigo-400">سحب {testUsedCash} / {employee.cashWithdrawLimit} د.ل</p>
                </div>
                <div className={`h-2 bg-[#0A0F1C] rounded-full overflow-hidden ${!employee.cashWithdrawEnabled && 'opacity-30'}`}>
              ...
              <div className="bg-indigo-500/10 border border-indigo-500/20 p-4 rounded-xl mt-6">
                <p className="text-xs font-bold text-indigo-300 flex items-center gap-2 mb-2"><CreditCard className="w-4 h-4" /> موقف السلف المتراكمة</p>
                <div className="flex justify-between items-center text-sm font-bold">
                  <span className="text-gray-400">إجمالي السلفة المقيدة</span>
                  <span className="font-mono text-white">{employee.totalAdvance.toLocaleString()} د.ل</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="pt-8 mt-8 border-t border-gray-800 flex justify-between">
          <button onClick={() => alert("سيتم إرسال بطاقة توقيع الكترونية (PIN) برسالة نصية.")} className="bg-indigo-500 hover:bg-indigo-400 text-white font-bold px-6 py-2.5 rounded-lg text-sm transition shadow-lg shadow-indigo-500/20">تعديل ملف وتجديد الـ PIN المقفل لـ ({'●'.repeat(employee.pin?.length || 4)})</button>
          <button onClick={onDelete} className="bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white font-bold px-6 py-2.5 rounded-lg text-sm transition flex items-center gap-2"><UserMinus className="w-4 h-4" /> فصل وإلغاء القيد</button>
        </div>
      </div>
    </div>
  );
...
function MenuItemsManagementView({ showSuccess }: { showSuccess: (m: string) => void }) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex items-center justify-between bg-[#121A2F] p-6 rounded-[2rem] border border-gray-800 shadow-sm">
        <div>
          <h3 className="text-xl font-extrabold text-white flex items-center gap-2"><Coffee className="w-6 h-6 text-orange-500" /> هندسة المنيو والتكاليف (Menu Engineering)</h3>
        ...
        {!isAdding && (
          <button onClick={() => setIsAdding(true)} className="bg-indigo-600 hover:bg-indigo-500 text-white font-bold px-6 py-3 rounded-xl shadow-lg shadow-indigo-500/20 transition flex items-center gap-2">
            إضافة صنف جديد / وصفة
          </button>
        )}
      </div>
      ...
      {isAdding && (
        <form onSubmit={handleAdd} className="bg-[#121A2F] border border-gray-800 rounded-3xl p-6 shadow-sm space-y-6">
          <h4 className="font-extrabold text-white">إضافة صنف جديد وتحديد التكلفة</h4>

          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
              <label className="text-xs font-bold text-gray-400 block mb-2">اسم الصنف</label>
              <input type="text" value={name} onChange={e => setName(e.target.value)} required placeholder="اسم الوجبة/المنتج" className="w-full bg-[#0A0F1C] border border-gray-700 focus:border-indigo-500 rounded-xl px-4 py-3 text-white outline-none" />
            </div>
            <div>
              <label className="text-xs font-bold text-gray-400 block mb-2">التصنيف</label>
              <div className="flex gap-2">
                <select value={category} onChange={e => { setCategory(e.target.value); setNewCategory(""); }} className="w-1/2 bg-[#0A0F1C] border border-gray-700 focus:border-indigo-500 rounded-xl px-4 py-3 text-white outline-none">
                  {existingCategories.map(c => <option key={c} value={c}>{c}</option>)}
                  <option value="NEW">تصنيف جديد...</option>
                </select>
                {category === "NEW" && (
                  <input type="text" value={newCategory} onChange={e => setNewCategory(e.target.value)} placeholder="اسم التصنيف..." required className="w-1/2 bg-[#0A0F1C] border border-gray-700 focus:border-indigo-500 rounded-xl px-4 py-3 text-white outline-none" />
                )}
              </div>
            </div>
            <div>
              <label className="text-xs font-bold text-gray-400 block mb-2">سعر البيع للزبون (د.ل)</label>
              <input type="number" step="0.25" value={price} onChange={e => setPrice(e.target.value)} required placeholder="0.00" className="w-full bg-[#0A0F1C] border border-gray-700 focus:border-indigo-500 rounded-xl px-4 py-3 text-white outline-none font-mono text-emerald-400 font-bold" />
            </div>
            <div>
              <label className="text-xs font-bold text-gray-400 block mb-2">أيقونة / صورة</label>
              <input type="text" value={image} onChange={e => setImage(e.target.value)} required className="w-full bg-[#0A0F1C] border border-gray-700 focus:border-indigo-500 rounded-xl px-4 py-3 text-white outline-none text-center text-xl" />
            </div>
          </div>

          <div className="border border-gray-800 rounded-2xl p-5 bg-[#0A0F1C]/50">
            <h5 className="text-sm font-bold text-gray-300 mb-4 flex items-center gap-2">مكونات الوصفة من المخزن (لحساب التكلفة)</h5>
            ...
            <div className="flex gap-4 mb-4">
              ...
              }} className="flex-1 bg-[#121A2F] border border-gray-700 rounded-xl px-4 py-2.5 text-white outline-none text-sm">
              ...
              <input type="number" step="0.01" value={selQty} onChange={e => setSelQty(e.target.value)} placeholder="الكمية" className="w-24 bg-[#121A2F] border border-gray-700 rounded-xl px-4 py-2.5 text-white outline-none text-sm font-mono" />

              <select value={selUnit} onChange={e => setSelUnit(e.target.value)} className="w-24 bg-[#121A2F] border border-gray-700 rounded-xl px-4 py-2.5 text-white outline-none text-sm">
                {selInvId && (() => {
                  const item = inventoryItems.find(i => i.id === Number(selInvId));
              ...
              <button type="button" onClick={addIngredient} className="bg-indigo-600 hover:bg-indigo-500 text-white font-bold px-4 rounded-xl text-sm transition">إضافة للصنف</button>
            </div>

            {ingredients.length > 0 && (
              <div className="bg-[#121A2F] rounded-xl overflow-hidden border border-gray-800">
                <table className="w-full text-right text-sm">
                  ...
                  <tbody className="divide-y divide-gray-800">
                    {ingredients.map((ing, idx) => (
                      <tr key={idx}>
                        <td className="py-2 px-4 text-gray-300">{ing.name}</td>
          ...
          <div className="flex items-center justify-between bg-emerald-500/10 border border-emerald-500/20 p-4 rounded-2xl">
            ...
            <div className="flex items-center gap-3">
              <button type="submit" className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold px-8 py-3 rounded-xl transition">تأكيد وحفظ الصنف</button>
              <button type="button" onClick={() => setIsAdding(false)} className="bg-gray-800 hover:bg-gray-700 text-white font-bold px-6 py-3 rounded-xl transition">إلغاء</button>
            </div>
          </div>
        </form>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {menuItems.map(item => {
          ...
          return (
            <div key={item.id} className={`bg-[#121A2F] border ${item.isAvailable ? 'border-gray-800' : 'border-red-900/30 opacity-70'} rounded-3xl p-5 hover:shadow-lg transition flex flex-col`}>
              <div className="flex items-center gap-4 mb-4">
                <div className={`w-16 h-16 rounded-2xl flex items-center justify-center text-3xl shrink-0 shadow-inner ${item.isAvailable ? 'bg-[#0A0F1C]' : 'bg-red-950/20 grayscale'}`}>
                  {item.image}
                </div>
                <div className="flex-1">
                  <h4 className="font-extrabold text-white text-base">{item.name}</h4>
                  <p className="text-[10px] font-bold text-indigo-400 mb-1">{item.category}</p>
              ...
              <div className="bg-[#0A0F1C] rounded-xl p-3 mb-4 flex justify-between items-center text-xs border border-gray-800">
                <div>
                ...
                <div className="text-left border-l border-gray-800 pl-3">
              ...
              <div className="flex gap-2 mt-auto">
                ...
                <button onClick={() => deleteItem(item.id!)} className="text-[11px] font-bold px-4 py-2 rounded-xl border bg-gray-800 text-gray-400 border-gray-700 hover:text-white hover:bg-gray-700 transition">
                  حذف
                </button>
              </div>
            </div>
...
function InventoryManagementView({ showSuccess }: { showSuccess: (m: string) => void }) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex items-center justify-between bg-[#121A2F] p-6 rounded-[2rem] border border-gray-800 shadow-sm">
        <div>
          <h3 className="text-xl font-extrabold text-white flex items-center gap-2"><Package className="w-6 h-6 text-indigo-500" /> إدارة المخازن والمواد الخام</h3>
      ...
      {isAdding && (
        <form onSubmit={handleAdd} className="bg-[#121A2F] border border-gray-800 rounded-3xl p-6 shadow-sm space-y-6">
          <h4 className="font-extrabold text-white">إضافة مادة خام جديدة</h4>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label className="text-xs font-bold text-gray-400 block mb-2">اسم المادة</label>
              <input type="text" value={name} onChange={e => setName(e.target.value)} required placeholder="مثال: دجاج كامل مبرد" className="w-full bg-[#0A0F1C] border border-gray-700 focus:border-indigo-500 rounded-xl px-4 py-3 text-white outline-none" />
            </div>
            <div>
              <label className="text-xs font-bold text-gray-400 block mb-2">التصنيف</label>
              <div className="flex gap-2">
            ...
            <div>
              ...
              <div className="flex gap-2">
                <select value={unit} onChange={e => { setUnit(e.target.value); setNewUnit(""); }} className="w-1/2 bg-[#0A0F1C] border border-gray-700 focus:border-indigo-500 rounded-xl px-4 py-3 text-white outline-none">
                  {existingUnits.map(u => <option key={u} value={u}>{u}</option>)}
                  <option value="NEW">وحدة جديدة...</option>
                </select>
                {unit === "NEW" && (
                  <input type="text" value={newUnit} onChange={e => setNewUnit(e.target.value)} placeholder="اسم الوحدة..." required className="w-1/2 bg-[#0A0F1C] border border-gray-700 focus:border-indigo-500 rounded-xl px-4 py-3 text-white outline-none" />
                )}
              </div>
            </div>
            <div>
              <label className="text-xs font-bold text-gray-400 block mb-2">الكمية الافتتاحية</label>
              <input type="number" step="0.1" value={quantity} onChange={e => setQuantity(e.target.value)} required placeholder="0" className="w-full bg-[#0A0F1C] border border-gray-700 focus:border-indigo-500 rounded-xl px-4 py-3 text-white outline-none font-mono" />
            </div>
            <div>
              <label className="text-xs font-bold text-gray-400 block mb-2">حد التنبيه (النواقص)</label>
              <input type="number" step="0.1" value={minAlert} onChange={e => setMinAlert(e.target.value)} required placeholder="0" className="w-full bg-[#0A0F1C] border border-gray-700 focus:border-indigo-500 rounded-xl px-4 py-3 text-white outline-none font-mono" />
            </div>
            <div>
              <label className="text-xs font-bold text-gray-400 block mb-2">متوسط التكلفة للوحدة (د.ل)</label>
              <input type="number" step="0.01" value={avgPrice} onChange={e => setAvgPrice(e.target.value)} required placeholder="0.00" className="w-full bg-[#0A0F1C] border border-gray-700 focus:border-indigo-500 rounded-xl px-4 py-3 text-white outline-none font-mono text-emerald-400 font-bold" />
            </div>
          </div>

          <div className="flex items-center gap-3 border-t border-gray-800 pt-6 mt-6">
            <button type="submit" className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold px-8 py-3 rounded-xl transition">حفظ في المخزن</button>
            <button type="button" onClick={() => setIsAdding(false)} className="bg-gray-800 hover:bg-gray-700 text-white font-bold px-6 py-3 rounded-xl transition">إلغاء</button>
          </div>
        </form>
      )}

      <div className="bg-[#121A2F] rounded-3xl border border-gray-800 overflow-hidden shadow-sm">
        <div className="overflow-x-auto">
          <table className="w-full text-right text-sm">
            <thead className="bg-[#0A0F1C]/80 border-b border-gray-800">
              <tr>
                <th className="py-4 px-6 font-extrabold text-gray-300">اسم المادة</th>
                <th className="py-4 px-6 font-extrabold text-gray-300">التصنيف</th>
                <th className="py-4 px-6 font-extrabold text-gray-300">الكمية المتاحة</th>
                <th className="py-4 px-6 font-extrabold text-gray-300">متوسط التكلفة</th>
                <th className="py-4 px-6 font-extrabold text-gray-300">إجمالي القيمة</th>
                <th className="py-4 px-6 font-extrabold text-gray-300 text-left">إجراءات</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-800/50">
              {inventoryItems.map(item => {
                ...
                return (
                  <tr key={item.id} className="hover:bg-[#0A0F1C]/50 transition group">
                    <td className="py-4 px-6 text-white font-bold">{item.name}</td>
                    <td className="py-4 px-6 text-indigo-300 text-xs">{item.category}</td>
                    ...
                    <td className="py-4 px-6 font-mono text-gray-300">{item.avgPrice.toFixed(2)} د.ل/{item.unit}</td>
                    ...
                    <td className="py-4 px-6 text-left flex gap-2 justify-end">
                      <button onClick={() => { setSupplyItem(item); setSupplyQty(""); setSupplyPrice(""); }} className="text-xs font-bold text-indigo-400 hover:text-white hover:bg-indigo-500 px-3 py-1.5 rounded-lg border border-indigo-500/20 transition opacity-0 group-hover:opacity-100">
                        توريد كمية
                      </button>
                      <button onClick={() => deleteItem(item.id!)} className="text-xs font-bold text-red-500 hover:text-white hover:bg-red-500 px-3 py-1.5 rounded-lg border border-red-500/20 transition opacity-0 group-hover:opacity-100">
                        حذف
                      </button>
                    </td>
                  </tr>
      ...
      {supplyItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setSupplyItem(null)}>
          <motion.div
            ...
            className="bg-[#121A2F] border border-indigo-500/30 rounded-3xl p-8 shadow-2xl shadow-indigo-500/10 w-full max-w-md mx-4"
          >
            <div className="flex justify-between items-start mb-6">
              <div>
                ...
                <p className="text-xs font-bold text-indigo-400 mt-1">{supplyItem.name}</p>
              </div>
            ...
            <div className="bg-[#0A0F1C] rounded-2xl p-4 mb-6 flex gap-6 text-sm font-bold">
              <div>
                <p className="text-gray-500 text-[10px] uppercase tracking-wider mb-1">الرصيد الحالي</p>
                <p className="font-mono text-white">{supplyItem.qty} <span className="text-gray-500">{supplyItem.unit}</span></p>
              </div>
              <div>
            ...
            <form onSubmit={handleSupply} className="space-y-5">
              <div>
                ...
                <input
                  ...
                  required
                  placeholder="0"
                  className="w-full bg-[#0A0F1C] border border-gray-700 focus:border-indigo-500 rounded-xl px-4 py-3 outline-none font-mono text-lg text-white"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  required
                  placeholder="0.00"
                  className="w-full bg-[#0A0F1C] border border-gray-700 focus:border-indigo-500 rounded-xl px-4 py-3 outline-none font-mono text-lg text-emerald-400 font-bold"
                />
              </div>

              {supplyQty && supplyPrice && (
                <div className="bg-indigo-500/10 border border-indigo-500/20 rounded-xl p-4 text-xs font-bold">
                  <p className="text-gray-400 mb-1">المتوسط المرجح الجديد المُتوقع:</p>
                  <p className="font-mono text-indigo-300 text-base">
              ...
              <div className="flex gap-3 pt-2">
                <button type="submit" className="flex-1 bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-3.5 rounded-xl transition shadow-lg shadow-indigo-600/20">
                  تأكيد الاستلام والتوريد
                </button>
                <button type="button" onClick={() => setSupplyItem(null)} className="px-6 py-3.5 bg-gray-800 hover:bg-gray-700 text-white font-bold rounded-xl transition">
                  إلغاء
                </button>
              </div>
            </form>
